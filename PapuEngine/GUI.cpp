...
CEGUI::Window* GUI::createWidget(const std::string& scheme, const glm::vec4& desctRect, const glm::vec4& desctPix, const std::string& name) {
	CEGUI::Window* newWindow = CEGUI::WindowManager::getSingleton().createWindow(scheme, name);
	_root->addChild(newWindow);
	setWidgetDestrect(newWindow, desctRect, desctPix);
	return newWindow;
}

void GUI::setWidgetDestrect(CEGUI::Window* newWindow, const glm::vec4& desctRect, const glm::vec4& desctPix) {
	newWindow->setPosition(CEGUI::UVector2(CEGUI::UDim(desctRect.x, desctPix.x), CEGUI::UDim(desctRect.y, desctPix.y)));
	newWindow->setSize(CEGUI::USize(CEGUI::UDim(desctRect.z, desctPix.z), CEGUI::UDim(desctRect.w, desctPix.w)));
}
